...
for k=1:S          % This for loop with index k calculates the fitness of each strain in the system i.e. w_k for k in [1,S] as defined in the text.
   ...
   -------------------------------------------------------------------------------------------------------------------------------------------------------------------
   %%%This block of code calculates the fitness of the k-th strain.
   
   
   Mk = Mv(any(Mv==k,2),:);        % Mk outputs an array showing which column of x (which ODE) contains strain k cells.     
      for ii=1:length(Mk(:,1))     % This for loop with index ii calculates the fitness of an individual strain.
     ...
     ----------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      w(k)=wk;                   % writes the fitness of k-th strain into a vector w. 
end
